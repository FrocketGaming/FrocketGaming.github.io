...
| Canary | `#ffd43b` → `#5a9dd6` (yellow → blue) | The Snake Charmer newsletter's own palette on a warm near-black ground (`#0c0b09`; every neutral, text and border included, carries a slight warm tint so the yellow reads as native). Syntax colors come from the theme's own hues (violet keywords, mint strings and additions, yellow functions, orange numbers, cyan types, blue variables) instead of a borrowed palette, and `--info-color` is cyan so it stays distinct from the blue accent. The only theme that overrides `--band-3` and `--band-4`, and the default for `/newsletter/` when a visitor has no saved theme |
