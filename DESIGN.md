...
- **Display** (JetBrains Mono / code stack, 800 weight, 72px; 48px on phones for the band numerals): the newsletter page's masthead (`.nl-masthead`, which shrinks with its container so `__dunder__ Review` stays on one line) and section-band numerals only (`newsletter/`). One step above Stat, used for a single dominant word or numeral per view, never for UI text.
